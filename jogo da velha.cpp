...
int verificarVencedor(char matriz[3][3], int *placarA, int *placarB) {
...
void realizarJogada(char matriz[3][3], char *simbolo, int linha, int coluna, char contra) {
...
void jogadaMaquina(char matriz[3][3]) {
    int linha, coluna;
    do {
        linha = rand() % 3;
        coluna = rand() % 3;
	...
	do{
		...
		switch (opcao){
			case 1:
				...
				break;
			case 2:
 				exibirRanking(jogador1, jogador2, placarA, placarB);
				break;
			case 3:
				//OPÇÃO DE CRÉDITOS
				    printf("===========================================\n");
